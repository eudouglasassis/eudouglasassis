# Desenvolvedor Web

## :wave: Bem-vindo
### :dart: Meu objetivo é ser um facilitador e solucionador de problemas utilizando a Tecnologia.

 :books:  Cursando Análise e Desenvolvimento de Sistemas na Universidade Presbiteriana Mackenzie.
 <br/>  :rocket: Fundamentos sólidos em: HTML5 | CSS3 | JavaScript | Bootstrap | Git | Linux.
 <br/>  :fire: Estudante assíduo de: Algoritmos | Estrutura de Dados | Arquitetura de Software.
 <br/>  :mag_right: Interesse em: SASS | TypeScript | NodeJS | PostgreSQL | MongoDB | Python.
 <br/>   :earth_americas: Idiomas: Inglês técnico e Português nativo.
 <br/>  :mortar_board: Formação: Bacharel em Engenharia civil.
 
:envelope: Entre em contato:  eudouglasassis@gmail.com
 
